...
def get_baselines():
    ...
    for metal in CATIONS:
        ...
        if metal == "Cu": 
            # Tenorite baseline for CuO
            struct = Structure.from_spacegroup(15, Lattice.from_parameters(4.68, 3.42, 5.12, 90, 99.5, 90), ["Cu", "O"], [[0.25, 0.25, 0], [0, 0.41, 0.25]])
        elif metal == "Zn": 
            # Wurtzite baseline for ZnO
            ...
            # Standard Rock salt baseline for all other A2+O2- oxides
...
# --- 9. SUMMARY DATA EXPORT ---
print("\n--- Exporting Summary CSV ---")
summary_data = {
    "Composition": ["".join(CATIONS)],
    "dH_mix_meV_atom": [dH_mix],
    "Mean_Bond_Length_A": [mean_b],
    "Std_Dev_Bond_Length_A": [sigma_b]
}
df_summary = pd.DataFrame(summary_data)
summary_path = os.path.join(output_folder, f"Summary_{''.join(CATIONS)}.csv")
