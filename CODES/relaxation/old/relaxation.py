...
# --- 3. UNIVERSAL PHASE-ACCURATE BASELINES ---
def get_baselines():
    print("\n--- Calculating Ground-State Phase Baselines ---")
    energies = []
    for metal in CATIONS:
        print(f"  > Relaxing pure {metal} oxide baseline...")
        if metal == "Cu": struct = Structure.from_spacegroup(15, Lattice.from_parameters(4.68, 3.42, 5.12, 90, 99.5, 90), ["Cu", "O"], [[0.25, 0.25, 0], [0, 0.41, 0.25]])
        elif metal == "Zn": 
            struct = Structure.from_spacegroup(186, Lattice.from_parameters(3.25, 3.25, 5.2, 90, 90, 120), ["Zn", "O"], [[1/3, 2/3, 0], [1/3, 2/3, 0.382]])
            atoms = adaptor.get_atoms(struct)
            atoms.calc = CHGNetCalculator(model)
            BFGS(atoms, logfile='-').run(fmax=1.0) 
            BFGS(FrechetCellFilter(atoms), logfile='-').run(fmax=FMAX_TOLERANCE)
            energies.append(atoms.get_potential_energy() / len(atoms))
            continue
        elif metal == "Pd": struct = Structure.from_spacegroup(131, Lattice.from_parameters(3.04, 3.04, 5.33, 90, 90, 90), ["Pd", "O"], [[0,0,0], [0,0.5,0.25]])
        elif metal == "Cr": struct = Structure.from_spacegroup(167, Lattice.hexagonal(4.96, 13.6), ["Cr", "O"], [[0,0,0.34], [0.30,0,0.25]])
        elif metal == "Ce": struct = Structure.from_spacegroup("Fm-3m", Lattice.cubic(5.41), ["Ce", "O"], [[0,0,0], [0.25,0.25,0.25]])
        elif metal == "Mn": struct = Structure.from_spacegroup("Fm-3m", Lattice.cubic(4.44), ["Mn", "O"], [[0, 0, 0], [0.5, 0.5, 0.5]])
        elif metal == "Ti": struct = Structure.from_spacegroup("P42/mnm", Lattice.from_parameters(4.59, 4.59, 2.95, 90, 90, 90), ["Ti", "O"], [[0,0,0], [0.3,0.3,0]])
        elif metal == "Al": struct = Structure.from_spacegroup(167, Lattice.hexagonal(4.75, 12.99), ["Al", "O"], [[0, 0, 0.352], [0.306, 0, 0.25]])
        elif metal == "Fe": struct = Structure.from_spacegroup(167, Lattice.hexagonal(5.03, 13.74), ["Fe", "O"], [[0, 0, 0.35], [0.30, 0, 0.25]])
        elif metal == "Mo": 
            struct = Structure(Lattice.cubic(3.85), ["Mo", "O", "O", "O"],[[0, 0, 0], [0.5, 0, 0], [0, 0.5, 0], [0, 0, 0.5]])
            atoms = adaptor.get_atoms(struct)
            atoms.calc = CHGNetCalculator(model)
            BFGS(FrechetCellFilter(atoms), logfile='-').run(fmax=FMAX_TOLERANCE)
            energies.append(atoms.get_potential_energy() / len(atoms))
            continue
        else: struct = Structure.from_spacegroup("Fm-3m", Lattice.cubic(4.25), [metal, "O"], [[0, 0, 0], [0.5, 0.5, 0.5]])
...
write_global_cif(relaxed_struct, moms_per_atom, os.path.join(output_folder, f"Global_magmoms_{''.join(CATIONS)}.cif"))
...
print("\n--- Exporting standard CIF files ---")

# Unrelaxed structure
unrelaxed_cif = os.path.join(
    output_folder,
    f"Unrelaxed_{''.join(CATIONS)}.cif"
)

# Relaxed structure
relaxed_cif = os.path.join(
    output_folder,
    f"Relaxed_{''.join(CATIONS)}.cif"
)

# Write standard CIFs with proper occupancies
heo.to(
    filename=unrelaxed_cif
)

relaxed_struct.to(
    filename=relaxed_cif
)
